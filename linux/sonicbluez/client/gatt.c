...
static void read_reply(DBusMessage *message, void *user_preamble)
{
	DBusError error;
	...
	if(user_preamble != NULL)
		rl_printf("%s%s\n",(char*)user_preamble,(char*)value);
	else
...
static void read_attribute(GDBusProxy *proxy, const char * arg)
{
