...
typedef struct iap_internal_state_
{
    ...
    // Pointer to the next byte in flash that will be written by iap_write.
...
iap_err_t iap_begin()
{
    ...
    // not allowed to call iap_begin if prev programming session is still open
    ...
    if (!iap_state.page_buffer) {
        ESP_LOGE(TAG, "iap_begin: not enough heap to allocate page buffer!");
    ...
    ESP_LOGD(TAG, "iap_begin: next boot partition is '%s'.",
						 iap_state.partition_to_program->label);
        
    iap_state.cur_flash_address = iap_state.partition_to_program->address;
    
    esp_err_t result = esp_ota_begin(iap_state.partition_to_program, 0, 
														&iap_state.ota_handle);
    if (result != ESP_OK) {
    ...
    ESP_LOGI(TAG, "iap_begin: open session for partition '%s', addr 0x%08x.",
             							iap_state.partition_to_program->label, 
												iap_state.cur_flash_address);
...
iap_err_t iap_write(uint8_t *bytes, uint16_t len)
{
    ...
    ESP_LOGD(TAG, "iap_write: cur_flash_address = 0x%08x", 
								iap_state.cur_flash_address);
    ...
    while (len > 0) {
        ...
        memcpy(&iap_state.page_buffer[iap_state.page_buffer_ix], bytes,
															 nofBytesToCopy);
...
iap_err_t iap_commit()
{
    ...
    if (result != IAP_OK) {
        ESP_LOGE(TAG, "iap_commit: programming session failed in last write.");
    }
    ...
    ESP_LOGI(TAG, "iap_commit: programming completed, partition activated.");
    return result;
}
...
static iap_err_t iap_write_page_buffer()
{
    ...
    esp_err_t result = esp_ota_write(iap_state.ota_handle,
							 iap_state.page_buffer, iap_state.page_buffer_ix);
    if (result != ESP_OK) {
        ESP_LOGE(TAG, 
		"iap_write_page_buffer: write failed in esp_ota_write (%d)!", result);
        return IAP_ERR_WRITE_FAILED;
    }
...
static iap_err_t iap_finish(int commit)
{
    ...
    if (commit) {
        ...
        if (result != ESP_OK) {
            ESP_LOGE(TAG, 
				"iap_finish: esp_ota_set_boot_partition failed (%d)!", result);
            return IAP_FAIL;
        }
    }
